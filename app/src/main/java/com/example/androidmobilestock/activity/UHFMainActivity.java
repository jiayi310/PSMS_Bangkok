...
import com.example.androidmobilestock.fragment.UHFReadTagFragment;
...
//	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		 super.onCreate(savedInstanceState);
			...
			adapter = new SimpleAdapter(UHFMainActivity.this, tagList, R.layout.listtag_items,
...
//			appContext = (AppContext) getApplication();
	...
	protected void onResume() {
		...
		st = getIntent().getParcelableExtra("STHeader");
		st_temp = getIntent().getParcelableExtra("STHeaderTemp");
		Log.wtf("First Activity Resume DocNo",st.getDocNo());
		...
		super.onActivityResult(requestCode, resultCode, data);
			if(requestCode == 1)
			{
				if(resultCode == 99) {
					...
					st_temp = data.getParcelableExtra("STHeaderTemp");
				}
				if(resultCode == 0) {
					...
					isSent = Integer.parseInt(data.getStringExtra("isSent"));
					...
					if(isSent == 1)
					{
						...
						st = data.getParcelableExtra("STHeader");
						st_temp = data.getParcelableExtra("STHeaderTemp");
						Log.wtf("First Activity DocNo",st.getDocNo());
						Log.wtf("StockDetailList Length", String.valueOf(st.StockDetailListLength()));
	...
	public AC_Class.StockTake gteDocNo()
	{
		return st;
	}

	public AC_Class.StockTake getSTTemp()
	{
		return st_temp;
	}
