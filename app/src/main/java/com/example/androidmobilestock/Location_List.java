...
import com.example.androidmobilestock.databinding.ActivityLocationListBinding;
...
public class Location_List extends AppCompatActivity {
    ...
    LocationListViewAdapter arrayAdapter;
...
//                startActivity(new_intent);
                    qrScan = new IntentIntegrator(Location_List.this);
    ...
    public void getLocationData() {
        Cursor data = db.getLocation();
        if (data.getCount() == 0) {
            return;
        }
        else if (data.getCount() > 0) {
            loc_inquiry.add(new AC_Class.Location("None", "null"));
            while (data.moveToNext()) {
                AC_Class.Location location =
                        new AC_Class.Location(data.getString(0), data.getString(1));
                loc_inquiry.add(location);
            }
            arrayAdapter = new LocationListViewAdapter(this, loc_inquiry);
    ...
    public void getItemData2(String substring) {
        Cursor data = db.getLocationLike(substring, 0);
        if (data.getCount() > 0){
            loc_inquiry.clear();
            loc_inquiry.add(new AC_Class.Location("None", "null"));
            while (data.moveToNext()) {
